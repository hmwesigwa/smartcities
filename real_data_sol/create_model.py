...
model.graph = nx.read_graphml("graph_data/lower_manhattan1_5.graphml")
for edge in model.graph.edges():
  u, v = edge
  model.graph[u][v]['time'] = model.graph[u][v]['weight']*0.00621371/model.graph.node[u]['speed_urban']*3.6 # 
  ...
  if node == 'source':
    ...
    if roadSeg == model.Routes[route_no][-1]:
      ...
      if node == write_data.roadSeg_to_node(roadSeg, model.nLayers.value):
        ...
        velocity = model.graph.node[str(roadSeg - 2)]['speed_urban']*0.01
        distance = model.graph.node[str(roadSeg - 2)]['length']*0.00621371
        INSTALL = True
