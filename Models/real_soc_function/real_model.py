...
for edge in model.graph.edges():
  u, v = edge
  model.graph[u][v]['time'] = model.graph[u][v]['weight']*0.000621371/model.graph.node[u]['speed_urban']*3.6 # 
  ...
  if node == 'source':
    ...
    if roadSeg == model.Routes[route_no][-1]:
      ...
      if node == write_data.roadSeg_to_node(roadSeg, model.nLayers.value):
        ...
        current_soc = 1
        velocity = model.graph.node[str(roadSeg - 2)]['speed_urban']
        distance = model.graph.node[str(roadSeg - 2)]['length']*0.000621371
        INSTALL = True
      ...
      next_start_node = write_data.roadSeg_to_node(next_roadSeg, model.nLayers.value)
      current_soc = write_data.node_to_soc(node, model.nLayers.value)
      node_posn = write_data.position_in_layer(current_soc, model.nLayers.value)
